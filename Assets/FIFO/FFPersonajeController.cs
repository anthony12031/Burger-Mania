...
public class FFPersonajeController : MonoBehaviour {
	...
	void Start () {
		...
		listos = GetComponent<PlanificadorFIFO> ().listos;
		suspendidos = GetComponent<PlanificadorFIFO> ().suspendidos;
		bloqueados = GetComponent<PlanificadorFIFO> ().bloqueados;
	...
	public void listoToBloqueado(int cpu,GameObject cliente)
	{
		...
		sizeBLO = GetComponent<PlanificadorFIFO>().bloqueados.Count();
